	...
	.then(
		function(err){
			if (err) {
				...
				quiz
				.save({fields: ["pregunta", "respuesta", "tema"]})  // guarda en DB los campos pregunta y respuesta de quiz
				.then(function(){ res.redirect('/quizes')})    // redirección HTTP (URL relativo) a la lista de preguntas
			}
		}
	);
};
